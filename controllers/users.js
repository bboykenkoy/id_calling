...
router.post('/signin', urlParser, function(req, res) {
    ...
    client.query(userSQL, function(error, data, fields) {
        if (error) {
            ...
            if (data.length > 0) {
                var token;
                auth.createAccessToken(req.body.key, 604800, function(access_token) {
                    token = access_token;
                    var insert = [];
            ...
            client.query(userSQL, function(error, data, fields) {
                if (error) {
                    ...
                    if (data.length > 0) {
                        ...
                        var dataSQL = "UPDATE `users_settings` SET " + update.toString() + " WHERE `users_key`='" + key + "'";
                        client.query(dataSQL, function(eInsert, dInsert, fInsert) {
                            if (eInsert) {
                                ...
                                if (req.body.is_visible == '1') {
                                    ...
                                    var dataSQLStaus = "UPDATE `users` SET `status` = 'online' WHERE `key`='" + key + "'";
                                    client.query(dataSQLStaus, function(eInsert, dInsert, fInsert) {
                                        console.log("Vừa update users_settings thành công cho users_key " + key);
                                        return res.send(echoResponse(200, 'Updated successfully', 'success', false));
                                    });
                                } else {
                                    console.log("status in visiale 0 ----------:" + req.body.is_visible);
                                    var dataSQLStaus = "UPDATE `users` SET `status` = 'offline' WHERE `key`='" + key + "'";
                                    client.query(dataSQLStaus, function(eInsert, dInsert, fInsert) {
                                        console.log("Vừa update users_settings thành công cho users_key " + key);
                                        return res.send(echoResponse(200, 'Updated successfully', 'success', false));
                                    });
